					...
					String methodName = ApptRandomTest.RandomSelectMethod(random);
					   if (methodName.equals("setTitle")){
						   ...
						   appt.setRecurrence(recurDays, recur, recurIncrement, recurNumber);
							 assertTrue(appt.getRecurNumber() == recurNumber);
							 assertTrue(appt.getRecurBy() == recur);
							 if(recurDays != null)
							 		assertTrue(appt.getRecurDays() == recurDays);
							 assertTrue(appt.getRecurIncrement() == recurIncrement);
						}
						else if (methodName.equals("isOn")) {
								int newHour=ValuesGenerator.getRandomIntBetween(random, -10, 24);
								int newMinute=ValuesGenerator.getRandomIntBetween(random, -10, 60);
								int newDay=ValuesGenerator.getRandomIntBetween(random, -10, 32);
								int newMonth=ValuesGenerator.getRandomIntBetween(random, -10, 13);
								int newYear=ValuesGenerator.getRandomIntBetween(random, -10, 2080);
								boolean ison = appt.isOn(newDay, newMonth, newYear);
